...
- [3. CONES](#3-cones)
  - [3.1. 팀명](#31-팀명)
  - [3.2. 로고](#32-로고)
  - [3.3. 핵심 키워드](#33-핵심-키워드)
  - [3.4. 세계관](#34-세계관)
  - [3.5. 콘셉트](#35-콘셉트)
    - [3.5.1. 프로젝트 음악](#351-프로젝트-음악)
    - [3.5.2. 프로젝트 메시지](#352-프로젝트-메시지)
  - [3.6. 비주얼](#36-비주얼)
- [4. UNIT](#4-unit)
  - [4.1. AI UNIT](#41-ai-unit)
    - [4.1.1. THINK](#411-think)
    - [4.1.2. PREDICT](#412-predict)
  - [4.2. COMPUTER UNIT](#42-computer-unit)
    - [4.2.1. BUILD](#421-build)
    - [4.2.2. EXECUTE](#422-execute)
- [5. 프로젝트](#5-프로젝트)
  - [5.1. 프로젝트 개요](#51-프로젝트-개요)
  - [5.2. 프로젝트 목표](#52-프로젝트-목표)
  - [5.3. 주요 기능](#53-주요-기능)
  - [5.4. 프로젝트 진행 과정](#54-프로젝트-진행-과정)
- [6. 콘텐츠](#6-콘텐츠)
  - [6.1. One Page Landing Page](#61-one-page-landing-page)
  - [6.2. GitHub](#62-github)
  - [6.3. Daily Snapshot](#63-daily-snapshot)
  - [6.4. 프로젝트 아카이빙](#64-프로젝트-아카이빙)
- [7. 기술](#7-기술)
  - [7.1. Tech Stack](#71-tech-stack)
  - [7.2. 개발 환경](#72-개발-환경)
  - [7.3. 협업 방식](#73-협업-방식)
  - [7.4. 프로젝트 구조](#74-프로젝트-구조)
  - [7.5. 티저 비디오 시스템](#75-티저-비디오-시스템)
  - [7.6. 배포](#76-배포)
  - [7.7. 접근성과 성능](#77-접근성과-성능)
- [8. 활동](#8-활동)
  - [8.1. 회의](#81-회의)
  - [8.2. 개발](#82-개발)
  - [8.3. 발표 및 행사](#83-발표-및-행사)
- [9. 기록](#9-기록)
  - [9.1. 업데이트](#91-업데이트)
  - [9.2. 성과](#92-성과)
- [10. 여담](#10-여담)
...
총 4명의 멤버로 구성되며, AI UNIT(THINK / PREDICT)과 COMPUTER UNIT(BUILD / EXECUTE)이 짝을 이뤄 하나의 순환 — THINK → PREDICT → BUILD → EXECUTE — 을 완성한다. 이는 곧 아이디어 → 분석 → 개발 → 실행으로 이어지는 팀의 실제 협업 구조이기도 하다.
...
CONES를 관통하는 키워드는 **CONNECTION**이다. 팀명, 세계관, 멤버 구성, 웹사이트 구조 전부 이 하나의 단어에서 파생된다.

- 이름: CON**nect** + ONES
- 세계관: 서로 다른 두 기원(AI UNIT, COMPUTER UNIT)의 연결
- 멤버: THINK → PREDICT → BUILD → EXECUTE, 네 명이 하나의 순환으로 연결
- 프로젝트명: `PROJECT : CONNECTION : 00`
- 웹사이트: 여러 페이지였던 구조를 하나로 연결한 One Page Landing Page
...
관측한 것을 해석하고, 아직 오지 않은 결과를 계산하는 유닛. THINK와 PREDICT, 두 단계를 담당한다.

<table>
<tr>
<td align="center" width="50%"><img src="public/images/rina-portrait-sm.jpg" width="140" alt="SERINA"></td>
<td align="center" width="50%"><img src="public/images/baesan-portrait-sm.jpg" width="140" alt="BAESAN"></td>
</tr>
<tr>
<td align="center"><b>THINK</b></td>
...
AI UNIT의 첫 단계. 관측하고 축적하는 역할로, 사이트에는 `LEARN`이라는 이름의 ABILITY로 구현되어 있다.
...
AI UNIT의 두 번째 단계. 축적된 패턴 위에서 갈라지는 미래를 계산한다. ABILITY 명칭도 동일하게 `PREDICT`.
...
- Position: MARKETING
- Role: 예측 / 분석 (`PREDICT`)
- Keyword: *"꿈이 크면 깨져도 크다"*

### 4.2. COMPUTER UNIT

판단을 구조로 옮기고, 구조를 현실에서 작동시키는 유닛. BUILD와 EXECUTE, 두 단계를 담당한다.

<table>
<tr>
<td align="center" width="50%"><img src="public/images/hyun-jizel-portrait-sm.jpg" width="140" alt="HYUN JIZEL"></td>
<td align="center" width="50%"><img src="public/images/ham-bom-portrait-sm.jpg" width="140" alt="HAM BOM"></td>
</tr>
<tr>
<td align="center"><b>BUILD</b></td>
<td align="center"><b>EXECUTE</b></td>
</tr>
</table>

#### 4.2.1. BUILD

COMPUTER UNIT의 첫 단계. 예측을 구조와 도면으로 옮기는 역할로, 사이트에는 `DESIGN`이라는 이름의 ABILITY로 구현되어 있다.
...
COMPUTER UNIT의 두 번째 단계. 설계를 현실에서 실제로 작동시킨다. ABILITY 명칭도 동일하게 `EXECUTE`.
...
- **One Page 스크롤 경험** — 히어로부터 CTA까지, 스크롤을 내리는 것만으로 CONES의 세계관을 처음부터 끝까지 훑는다.
- **창설 시퀀스** (`ConnectionSequence`) — 스크롤 위치에 실시간으로 반응하는 SVG로 두 원이 하나로 연결되는 순간을 연출한다.
- **시스템 순환** (`SystemCycle`) — LEARN → PREDICT → DESIGN → EXECUTE 네 단계를 하나의 루프로 시각화한다.
- **아티스트 프로필 모달** (`ArtistProfileModal`) — 멤버 카드를 눌러도 페이지 이동 없이 상세 프로필이 열린다.
- **티저 비디오 시스템** — 멤버별 티저 슬롯을 관리하며, 화면에 보일 때만 재생하고 벗어나면 즉시 정지한다.
- **D-Day 카운트다운** (`Countdown`) — 데뷔 쇼케이스까지 남은 시간을 실시간으로 보여준다.
- **접근성 대응** — `prefers-reduced-motion`에서 모든 애니메이션이 정지 상태로 축소되고, 시맨틱 마크업과 포커스 관리를 갖췄다.
- **네트워크 비의존 배포** — 외부 CDN·폰트 서비스 없이 정적 파일만으로 인터넷 연결 없이도 동작한다.
...
- 협업 방식에 대한 자세한 내용은 [7.3. 협업 방식](#73-협업-방식) 참고.
...
- README 구조 개편 — 위키형 프로젝트 문서 구조 적용, 멤버 사진 및 UNIT 프로필 카드 추가
...
React 19 + TypeScript(strict) + Vite로 개발되었으며, UI/애니메이션 라이브러리 없이 네이티브 CSS/DOM/SVG로 모든 인터랙션을 구현했다. 자세한 스택과 폴더 구조는 [7. 기술](#7-기술) 참고.

### 8.3. 발표 및 행사

데뷔 쇼케이스는 **2026.09.15**로 예정되어 있다. 발표·행사 관련 세부 내용은 추후 업데이트 예정.

## 9. 기록

### 9.1. 업데이트

프로젝트가 어떻게 발전해왔는지는 [5.4. 프로젝트 진행 과정](#54-프로젝트-진행-과정)과 [6.3. Daily Snapshot](#63-daily-snapshot)에 날짜순으로 기록된다. 이 문서를 계속 갱신하는 것 자체가 업데이트 기록이다.
